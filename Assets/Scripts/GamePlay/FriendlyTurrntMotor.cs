...
public class FriendlyTurrntMotor : MonoBehaviour {
	...
	void Start(){
		...
		RPlayer = GameObject.Find("Player").transform;
	...
	void Update () {
		enemyD = Vector3.Distance(Enemy.position, transform.position);
	...
	void Shoot(){
		...
		bullit.GetComponent<BulletScript>().Target = "Enemy";
